...
const url = 'https://demo.computervision.com.vn/api/v2/nlpextract/civil_registration?get_thumb=true'

export default function DemoSoKhaiSinh({ result, setResult }) {
  ...
  const onSubmit = (recaptchaToken) => {
    ...
    if (file) {
      ...
      axios({
        method: "post",
        url: `${window.location.origin}/api/ocr/v2?type=so-khai-sinh`,
        data: formData,
      })
        .then(res => {
          setResult(res.data)
      ...
      axios({
        method: "get",
        url: `${window.location.origin}/api/ocr/v2?type=so-khai-sinh&img=${imageUrl}&recaptchaToken=${recaptchaToken}`,
      })
        .then(res => {
          setResult(res.data)
          setLoading(false)
