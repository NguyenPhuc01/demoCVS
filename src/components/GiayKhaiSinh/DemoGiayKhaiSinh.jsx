...
export default function DemoGiayKhaiSinh({ result, setResult }) {
  ...
  const onSubmit = (recaptchaToken) => {
    ...
    if (file) {
      ...
      axios({
        method: "post",
        url: `${window.location.origin}/api/ocr/v2?type=giay-khai-sinh`,
        data: formData,
      })
        .then(res => {
          setResult(res.data)
      ...
      axios({
        method: "get",
        url: `${window.location.origin}/api/ocr/v2?type=giay-khai-sinh&img=${imageUrl}&recaptchaToken=${recaptchaToken}`,
      })
        .then(res => {
          setResult(res.data)
          setLoading(false)
