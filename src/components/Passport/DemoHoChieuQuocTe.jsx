...
const url = 'https://demo.computervision.com.vn/api/v2/ocr/passport?get_thumb=true'

export default function DemoHoChieuQuocTe({ result, setResult }) {
  ...
  const onSubmit = (recaptchaToken) => {
    ...
    if (file) {
      ...
      axios({
        method: "post",
        url: `${window.location.origin}/api/ocr/v2?type=passport`,
        data: formData,
      })
        .then(res => {
          setResult(res.data)
      ...
      axios({
        method: "get",
        url: `${window.location.origin}/api/ocr/v2?type=passport&img=${imageUrl}&recaptchaToken=${recaptchaToken}`,
      })
        .then(res => {
          setResult(res.data)
          setLoading(false)
