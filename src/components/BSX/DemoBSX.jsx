...
const url = 'https://demo.computervision.com.vn/api/v2/ocr/vehicle_plate?get_thumb=true'

export default function DemoBSX({ result, setResult }) {
  ...
  const onSubmit = (recaptchaToken) => {
    ...
    if (file) {
      ...
      axios({
        method: "get",
        url: `${window.location.origin}/api/ocr/v2?type=bsx&img=${imageUrl}&recaptchaToken=${recaptchaToken}`,
      })
        .then(res => {
          setResult(res.data)
          setLoading(false)
