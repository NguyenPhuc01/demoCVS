...
export default function DemoPhilippines({ result, setResult }) {
  ...
  const onSubmit = (recaptchaToken) => {
    ...
    if (file) {
      ...
      axios({
        method: "post",
        url: `${window.location.origin}/api/ocr/v2-cam?type=philippines`,
        data: formData,
      })
        .then(res => {
          setResult(res.data)
      ...
      axios({
        method: "get",
        url: `${window.location.origin}/api/ocr/v2-cam?type=philippines&img=${imageUrl}&recaptchaToken=${recaptchaToken}`,
      })
        .then(res => {
          setResult(res.data)
          setLoading(false)
