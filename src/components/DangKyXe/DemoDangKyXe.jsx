...
const url = 'https://demo.computervision.com.vn/api/v2/ocr/vehicle_registration?get_thumb=true'

export default function DemoDangKyXe({ result, setResult }) {
  ...
  const onSubmit = (recaptchaToken) => {
    ...
    if (file) {
      ...
      axios({
        method: "post",
        url: `${window.location.origin}/api/ocr/v2?type=dang-ky-xe`,
        data: formData,
      })
        .then(res => {
          setResult(res.data)
      ...
      axios({
        method: "get",
        url: `${window.location.origin}/api/ocr/v2?type=dang-ky-xe&img=${imageUrl}&recaptchaToken=${recaptchaToken}`,
      })
        .then(res => {
          setResult(res.data)
          setLoading(false)
